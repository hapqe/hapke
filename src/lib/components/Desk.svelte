...
Command: npx @threlte/gltf@2.0.3 static/Desk.glb
-->
...
<script>
  ...
  import { bounceIn, cubicInOut, cubicOut } from "svelte/easing";
  ...
  useFrame(({}, delta) => {
    ...
    // lerp swing based on plant cooldown
    ...
    <T.Group position={[0, 0, $position]} rotation={[0, $position * 2, 0]}>
      <T.Mesh
        ...
        on:pointerdown={(event) => {
          plant.set(Math.round(Math.random()) * 10 - 5 + Math.random() * 4 - 2);
          plantCooldown = 1.0;
        }}
      >
      ...
      <T.Group position={[0.04, -0.1, 0.32]} scale={0.09}>
        <T
          on:create={({ ref }) => {
            let relavant = ref.children[0];
            plant.subscribe((x) => {
              relavant.rotation.x = 0.2 * (0.2 - x);
            });
          }}
          is={gltf.nodes.Bone}
        />
      ...
      <T.Mesh
        ...
        on:pointerdown={(event) => {
          lamp.set(
            Math.round(Math.random()) * 1 - 0.5 + Math.random() * 0.3 - 0.14,
          );
        }}
      >
      ...
      >
        <T
          ...
          on:create={({ ref }) => {
            let relavant = ref.children[0];
            laptop.subscribe((x) => {
              relavant.rotation.z = -2 + 0.5 * (0.5 - x);
            });
          }}
        />
        <T.SkinnedMesh
      ...
      >
        <T
          on:create={({ ref }) => {
            let relavant = ref.children[0];
            chair.subscribe((x) => {
              relavant.rotation.z = -0.1 + 0.1 * (0.1 - x);
            });
          }}
